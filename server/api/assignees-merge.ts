...
import {dbAssigneeToFrontend} from '../db/mappers';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    await prisma.$transaction(async (tx) => {
        await tx.reservation.updateMany({
            where: {storeId, assigneeId: source.id},
            data: {assigneeId: target.id},
        });
        await tx.assignee.delete({where: {id: source.id}});
    });
    ...
    return res.json({ok: true, assignees: dbAssignees.map(dbAssigneeToFrontend)});
}
