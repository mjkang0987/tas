...
**표시는 글자가 아니라 디자인으로 한다** — 옅은 배경 틴트 + 테두리.
임시 휴업일=`--danger-bg`/`--danger-border`, 정기 휴무=`--neutral-bg`/`--neutral-border`.
테두리는 `border`가 아니라 **inset box-shadow** — border를 주면 셀·열 크기가 1px 밀려
달력 격자와 시간축이 어긋난다.
- `Month.tsx` — 월별 셀
...
글자 사이 어디서나 끊겨 '정/기/휴/무'로 쪼개진다. 틴트·테두리는 **폭을 먹지 않아 이 문제가 구조적으로 사라진다.**

**색만으로는 스크린리더에 아무것도 전달되지 않으므로** 문구(`STORE_CLOSED_LABEL`)를 전역 `.a11y`
클래스로 DOM에 남긴다. `title`(툴팁)은 **월별 셀에만** 단다 — 타임라인 래퍼는 하루 열 전체라
자식이 title을 물려받아 예약 카드 위에서도 '휴업일'이 뜬다.
...
| 뷰 | 결과 |
|---|---|
| 월별 1440px | 휴무 8칸(휴업일 2 + 일요일 6) 전부 틴트+테두리, 예약 있는 20일도 카드와 겹치지 않음 |
| 주별 1440px | 3개 열 표시, 틴트+테두리 육안 확인 |
| 일별 1440px | 하루 전체 틴트 + 테두리 |
...
### 리스크 / 남는 것
- 테두리가 옅은 톤(`--danger-border`/`--neutral-border`)이라 신호의 대부분은 배경 틴트가 진다.
  더 또렷하게 원하면 테두리 색만 진하게 올리면 된다.
- 실기기(Safari·iOS) 미확인 — 배경색·의사요소뿐이라 위험은 낮다.
- **앱(tas-ios)은 판정 로직·단위 테스트까지만** 넣었다. 화면 표시는 같은 B안(틴트+테두리)으로
  붙이면 되고(틴트+테두리), 배지 컴포넌트 논의는 무의미해졌다(글자를 안 쓰므로 새 컴포넌트가 필요 없다).
