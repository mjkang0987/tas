...
## 진행 중 — 네이버(=SNS) 첫 로그인 시 화면 여러 번 튐 완화

### 배경 / 진단
- 신고: "네이버 로그인하면 창 여러 번 튀면서 로그인됨"(데스크탑에서 관찰, 로그인 자체는 성공).
- 조사 결과 **네이버 전용 버그 아님**. 기존(온보딩 완료) 계정 재로그인은 곧장 `/month/…` 착지로 튐 없음.
- 여러 번 튀는 실체 = **신규 계정 첫 로그인의 게이트 체인**:
  `콜백 → /month/… → (미들웨어: 약관 미동의) /consent → (동의 후) /month/… → (미들웨어: 온보딩 미완료) /onboarding → /`.
  캘린더를 두 번 스쳐 가며 화면 전환이 늘어남(구글·카카오도 동일). 네이버 자체 OAuth 페이지 전환이 더해져 체감 심화.

### 변경 (저위험, 전 provider 공통)
- `client/pages/consent.tsx` `submitAuthConsent`: 동의 직후 세션 갱신 결과를 보고, **순수 SNS 신규 가입(게스트 데이터 이관 경로 제외)인데 온보딩 전**이면 캘린더(`resolveNextPath`) 대신 곧장 `/onboarding`으로 하드 이동. → `동의 → 캘린더 → 온보딩` 왕복에서 캘린더 경유 1회 제거.
- 게스트 이관 경로(`needsDpaOnly`)는 기존대로 캘린더로 보내 마이그레이션 레이어가 처리(회귀 방지).

### 검증
- 타입체크/빌드. 신규 가입 흐름(동의→온보딩 직행) + 기존 계정 재로그인(캘린더 직행) 회귀 없음 확인.
