...
처음엔 처리한 messageId 원장(`NaverSyncedMessage` 테이블)을 제안했으나, 사용자 제안대로 **타임스탬프 워터마크**로 갔다. 원장은 과했다.

워터마크 저장 위치로 두 안을 비교했다:

| | A: `GmailConnection.lastSyncedAt` | B: `MAX(Reservation.createdAt)` 유도 |
|---|---|---|
| 스키마 | 컬럼 1개 | 없음 |
| 외부 오염 | 불가 | **가능(치명)** |
| 스캔 범위 | 항상 1일 | 마지막 네이버 예약 이후 **전체** |

**B를 버린 이유** — `ReservationDetailSections.tsx:196`에 `<option value="네이버예약">`이 있어 **오너가 예약 채널을 수동으로 바꿀 수 있다.** `createdAt`은 행 생성 시각이므로, 오늘 등록한 전화예약을 네이버예약으로 바꾸면 워터마크가 오늘로 점프하고 그보다 오래된 미동기화 메일이 영구 유실된다. 동기화 정확성이 오너가 UI에서 만지는 데이터에 매달리는 구조. 또 B는 네이버 예약이 새로 생길 때만 전진하므로(취소는 예약을 안 만든다) 예약이 끊긴 매장은 범위가 무한히 커져 목적 자체를 달성하지 못한다.

### 구현 (A)

- 마이그레이션 `0020_gmail_connection_last_synced_at` — 가산·nullable·`IF NOT EXISTS`. **운영엔 사용자가 수동 선적용 완료.**
- **저장값 = 실행 '시작' 시각.** 종료 시각을 쓰면 실행 중 도착한 메일이 `messages.list` 스냅샷에 없었으므로 다음 범위에서 빠진다.
- **조회 = `lastSyncedAt - 1일`.** `after:`는 Gmail **수신 시각** 기준이고 워터마크는 실행 시작 시각이므로, 못 본 메일의 수신 시각은 항상 워터마크보다 뒤다 → 발신 지연·SMTP 재시도는 무관하고 **색인 지연만** 남는다(초~분 단위). 3일에서 1일로 줄였다.
- **`now`로 상한 클램프** — 시계 문제나 잘못된 쓰기로 `lastSyncedAt`이 미래가 되면 조회 범위가 비어 동기화가 **알림도 없이 영구 정지**한다.
- `NULL`이면 종전대로 이번 달 1일 → 백필 불필요.

### 전진 조건 — 여기가 유실 방지의 핵심

`errors.length === 0` 만으로는 부족하다. `listMessageIds`가 페이지 실패 시 부분 결과를 조용히 반환했기 때문에, 완전성을 명시적으로 신고하게 바꿨다(`{ids, complete}`).
...
쿨다운이 특히 중요하다 — `isRateLimited()`가 빈 목록을 반환하는데, 이걸 '변화 없음'으로 오해해 전진시키면 쿨다운 구간 메일이 통째로 범위 밖으로 나간다.

### 검증
- `pnpm build`·`tsc --noEmit`·`prisma validate` 그린
- fetch 스텁 런타임 검증 24개 검사 통과 — 재시도/원인구분 9개 + `complete` 의미론(정상 true, 페이지 실패 false, 상한 false, 쿨다운 false)

### 효과
스캔 범위 **29일치 → 1일치**. 월중 어느 시점이든 일정.

---

## 완료 — 네이버 메일 동기화 안정화 1단계 (#177 #178, 에픽 #176)

> 계기: 2026-07-29 ops 채널 `🛑 네이버 동기화 실패` 3건.

### 원인 규명

Gmail 메시지 ID는 `id >> 20`이 ms 에포크다. 디코드하면 실패한 셋 다 **새로 온 메일이 아니었다** — 각각 11일·4일·1일 전 도착. `getLastNaverSyncTimestamp()`가 항상 이번 달 1일부터 재스캔하므로 이 메일들은 이미 수백 번 정상 fetch돼 왔다. 여기서 후보가 걸러진다:

| 가설 | 판정 |
|---|---|
| HTML 파트 없음 | **탈락** — 메시지 고유 속성이라 결정적. 매 폴링 실패했어야 하는데 1회만 실패 |
| 404(삭제) | **탈락** — 삭제되면 `messages.list`에 안 잡힘. 게다가 16:00 실패 건이 16:48엔 성공 |
| 401(토큰 만료) | **탈락** — 실행 중 401이면 이후 전부 연쇄 실패. 2건/1건으로 끝날 수 없음 |
| 429 | **탈락** — `handleRateLimit`이 15분 쿨다운을 걸어 남은 전부가 실패했어야 함 |
| **403 rateLimit 계열 / 일시적 5xx** | **부합** — 쿨다운을 안 걸어 건별로 흩어져 실패. 관측 패턴 그대로 |

**결론: `messages.get` burst 스로틀 + 재시도 부재.** 월말일수록 재스캔 대상이 누적되고(29일=약 4주치) 예약·취소 배치를 `Promise.all`로 돌려 실동시성이 20이라, burst가 월중 선형으로 커져 월말에 발현했다.

### 구현 (스키마 변경 없음 — 커밋 단위로 분리)

1. **실패 원인 구분** — `getEmailContent` 반환을 `EmailFetchResult`(`http`/`no_html_part`/`network`/`cooldown`)로. Google 에러 `reason`을 detail로 추출, 로그에 messageId 포함.
2. **지수 백오프 재시도** — 400ms/1.2s/3s 3회. 403은 사유로 갈라 `rateLimit` 계열만 재시도(`insufficientPermissions`는 스코프 문제라 즉시 포기). **429는 재시도하지 않음** — 쿨다운이 호출부 전체를 멈추는 게 맞다.
3. **실동시성 20 → 10** — 두 배치를 순차 실행.
4. **`nextPageToken` 순회** — 첫 페이지만 읽어 월 500건 초과 매장은 오래된 메일이 잘렸다. 상한 10페이지, 중간 실패 시 **부분 결과 보존**(전부 버리면 폴링이 통째로 빈다).
5. **P2002 제약 구분** — `err.meta.target`으로 `naverBookingId` 중복(정상 skip)과 legacyId 충돌을 분리. 후자는 동기화 동시 실행이 같은 번호를 발급한 경쟁 상황이라 **예약이 누락되는데 `errors`에도 안 잡혀 보이지 않았다.**
6. **알림 개선** — 매장명 prefix, 취소 메일 실패 건수 별도 경고(유령 예약으로 이어지는 유일한 경로), 알림 전송 오류 격리.

### 검증
- `pnpm build` 그린, `tsc --noEmit` 그린.
- fetch 스텁 런타임 검증 7종 통과 — 403 재시도 후 성공/백오프 대기, `insufficientPermissions` 무재시도, 404 무재시도, `no_html_part` 구분, 503 재시도 소진(무한루프 아님), 페이지네이션 수집, 목록 중간 실패 시 부분 보존.

### 남은 작업
- **#179 처리한 messageId 원장** (스키마 변경) — 근본 해결. 중복 판정이 `existingBookingMap`인데 bookingId는 HTML을 파싱해야 나오므로, 이미 처리한 메일도 매번 본문을 받아온 뒤에야 버린다. 원장을 두면 fetch 전에 거를 수 있다. **마이그레이션 수동 선적용 → 머지 순서 준수.**
- 에픽 범위 밖: 서버 크론 이관(지금은 오너가 브라우저를 열어야만 동기화 → 주말에 취소 미반영), Gmail push(`users.watch`+Pub/Sub).

---

## 완료 — 예약추가 레이어 고객명 한글 오름차순 정렬 (#174, v0.44.1)

> 요청(사용자): 예약추가 레이어 내 고객명 뜰 때 한글 오름차순 정리.

### 배경
- 예약추가 레이어(`ReservationCreate`)의 고객명 자동완성 목록이 `Object.values(customerMap)` 순서, 즉 **고객 `legacyId` 오름차순(등록 순)** 으로 나왔다. 이름으로 훑어 찾기가 어렵다.

### 구현
- **`features/customers/model.ts`에 정렬 헬퍼 신설** — `compareCustomerName`(비교자) · `sortCustomersByName`(사본 정렬). 기준은 `Intl.Collator('ko', {numeric: true})`.
  - **코드포인트 비교(`a.name < b.name`)를 쓰지 않은 이유**: 한글·라틴·숫자가 섞이면 'Z'가 '가' 앞에 오는 등 사용자가 기대하는 가나다순이 아니다. `numeric: true`라 `고객2` < `고객10`.
  - **동명이인은 `id` 순으로 갈라 안정 정렬** — 이름만 비교하면 렌더마다 순서가 흔들릴 여지가 있다.
- **`useReservationCreateForm`**: `customers`를 `useMemo`로 정렬해 두고, 검색 필터는 그 배열을 걸러 순서를 그대로 물려받게 했다. `nextCustomerId`는 최댓값이라 순서와 무관 → 같은 배열 공유.
- **검색 매칭 규칙은 무변경**(이름/연락처 부분일치). 표시 컴포넌트(`CustomerAutocomplete`)도 무변경 — 받은 순서대로 그린다.

### 범위 밖(이번엔 손대지 않음)
- 같은 `CustomerAutocomplete`를 쓰는 **회원권 발급·적립금 조정** 화면은 요청 범위(예약추가)가 아니라 그대로 뒀다. 헬퍼는 공용이라 필요할 때 한 줄로 적용 가능.

### 검증 (완료)
- ✅ `tsc --noEmit` 0 · `next build` 성공.
- ✅ **실브라우저(Playwright, 게스트 모드 로컬 스냅샷)** — 등록 순과 가나다순이 어긋나도록 고객 9명을 섞어 넣고 예약추가 레이어를 열어 확인.
  - 포커스 직후 전체 목록: `강감찬 → 고객2 → 고객10 → 김철수(010-2222) → 김철수(010-6666) → 김하늘 → 박지민 → 홍길동 → Anna`. 가나다순·숫자순·동명이인 id순·라틴 후순위 모두 기대대로.
  - `김` 검색 → 3건이 같은 순서 유지. `0106`(연락처) 검색 → 해당 1건. 페이지 에러 0.

### 리스크
- 스키마·API·마이그레이션 **무변경(코드-온리 배포)**. 배포 순서 제약 없음.
...
### 1. 고객 병합 미리보기에 판단 정보
- 배경: 미리보기가 **이름·연락처·기준/삭제 배지**만 보여줘, 동명이인이나 마스킹된 이름(`*` 포함, 네이버 유입)이 섞이면 기준 고객을 고를 근거가 없었다. 기준 고객의 이름·연락처가 남으므로 잘못 고르면 분리 전까지 틀린 번호가 유지된다.
- `AddressContent.tsx` 미리보기 항목에 **예약 건수 · 적립금** 추가. 이미 받고 있던 `reservationsByCustomer`·`customer.points`로 충족 — API·스키마 무변경.
- 연락처 포맷 인라인 정규식(11자리 전용) → 공용 `formatTel`. 10자리 번호도 정상 표기.

### 2. 고객명단 최근 서비스 레이아웃
- 모바일(≤840px)에서만 최근 서비스를 이름·연락처 줄과 분리해 별도 줄로. 시술명은 공백에서 쪼개지지 않게(`white-space: nowrap`), 칩은 칩 단위로 줄바꿈.
- **데스크톱(≥841px)은 작업 이전 배치 그대로** — DOM은 형제로 옮겼지만 미디어쿼리로 기존 렌더를 재현(`flex: 1` + `overflow: hidden`).

### 3. 모바일 하단 탭바 터치 영역
- 상하 여백을 `StyledTabBar` → 각 `StyledTab`(링크)으로 이관. 여백이 링크 밖 죽은 공간이던 문제 해소.
- 비율은 상 10px : 하 15px(2:3). 하단은 홈 인디케이터 safe-area가 더 크면 그 값을 따르고 링크가 그 영역까지 덮는다.

### 4. 매출 모바일 기간 선택 겹침
- 증상: iOS에서 두 번째 날짜 입력이 "다음 기간" 버튼 위로 ~12px 겹침. 실제 박스 폭(171/183px)이 슬롯(165px)보다 넓었다.
- 원인: iOS Safari가 `input[type="date"]`에 UA 고유 폭·가운데 정렬을 적용해 `width: 100%`를 넘어서 그림.
- 수정: `revenue-styles.ts`의 `StyledDateInput`에 `appearance: none` + `::-webkit-date-and-time-value { margin: 0; text-align: left; }`. 좌우 이동 버튼은 `flex-shrink: 0`.

### 검증
- ✅ 각 단계 `next build` 성공.
- ⬜ **iOS 실기기 확인 필요** — 4번은 iOS 전용 렌더 문제라 로컬에서 재현·검증 불가. 배포 후 아이폰에서 매출 기간 선택 겹침 해소·날짜 피커 동작 확인할 것.
- ⬜ 고객명단·병합 미리보기 실제 화면 확인(로그인·데이터 필요).

## 완료 — 고객 개인정보 정리 (예약 페이지 이전 후속)

> 검증(2026-07-28): 운영에서 `Customer.allergyNote`·`claimNote`·`preferenceNote` 제거됨(마이그레이션 0018). 코드 참조도 0건.
...
## 완료 — 매장 공지사항 (StoreNotice, 예약 페이지 목록형 공지)

> 검증(2026-07-28): 운영에 `StoreNotice` 테이블·`pinned` 컬럼 존재(마이그레이션 0015·0016), 공지 1건 등록됨.
...
## 완료 — 온라인 예약 승인/거절/취소 UX 개선 (승인 확인 레이어·취소 예약 유지·사유)

> 검증(2026-07-28): 운영에 `Reservation.decisionReason` 존재(마이그레이션 0014), `ReservationDetail`에 사유 UI 반영됨.
...
## 완료 — 예약 상태별 오너 안내문구 (#139: 완료·확정·취소, 4개 언어)

> 검증(2026-07-28): 운영에 `doneText`·`confirmText`·`cancelText` + 각 i18n 컬럼 존재(마이그레이션 0013), 설정·고객 페이지 양쪽 반영됨.
...
## 완료 — 예약 페이지 오너 콘텐츠 다국어 (Phase A: 오너 입력 + 한국어 폴백)

> 검증(2026-07-28): 운영에 `nameI18nJson`(Service·Assignee·Store)·`noticeI18nJson` 존재(마이그레이션 0012), `LocalizedMessageField` 배포됨.
...
## 완료 — 취소/노쇼 예약 시각 분기 통일 (빗금 + 고객명 취소선)

> 검증(2026-07-28): `ReservationList`에 빗금(`repeating-linear-gradient`)·취소선(`.strike`) 반영됨.
...
## 완료 — 서비스 소요시간 분→시간+분 표기 (#126)

> 검증(2026-07-28): `utils/services.formatDuration`을 서비스 관리·예약 폼에서 사용 중.
...
## 완료 — 고객 공개 예약 시스템 (Online Booking)

> 검증(2026-07-28): 운영에 `Store.useOnlineBooking`·`bookingSlug`·`StoreBookingSettings`·`Reservation.publicToken` 존재(마이그레이션 0008·0009).
> 공개 페이지 `pages/book/[slug].tsx`, 예약 조회 `pages/book/[slug]/r/[token].tsx` 배포됨. 후속 항목(승인형 전환·다국어·안내문구)은 각 완료 섹션 참조.
...
## 완료 — `h3` 태그를 `strong`으로 교체

> 검증(2026-07-28): `<h3>`·`styled.h3` 잔존 0건.
...
## 완료 — 북마크(직접 URL 진입) 크래시 수정

> 검증(2026-07-28): `LayoutComponent`에 날짜 세그먼트 폴백 가드 반영됨.
...
> 검증(2026-07-28): 운영에 `CouponProduct`(1건)·`CustomerCoupon` 존재, `useCouponSystem` 켠 매장 1곳. 마이그레이션 0007·0017 적용됨.
...
## 완료 — 매장관리: 적립금/회원권 시스템 토글 + 회원권 풀 기능

> 검증(2026-07-28): 운영에 마이그레이션 0005·0006 적용, `MembershipManageSection` 노출. (회원권 발급 실사용 0건)
...
## 완료 — 1d 고객 확인·변경·취소 (오너 승인형) (#76)

> 검증(2026-07-28): 운영에 `Reservation.pendingAction`·`pendingPayloadJson`·`pendingRequestedAt` 존재(마이그레이션 0010), 고객 페이지 `pages/book/[slug]/r/[token].tsx` 배포됨.
...
## 완료 — 온라인 예약을 "신청→오너 확정형"으로 전환 (+슬롯 차단, 네이버 2중검증)

> 검증(2026-07-28): 운영 `ReservationStatus` enum에 `requested` 포함(마이그레이션 0011), 알림 벨 `BookingRequestNotification` 배포됨.
