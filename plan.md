...
### 배경
iOS 앱과 웹 모바일을 같은 시드로 나란히 띄워 비교했다. 둘 다 같은 하단 탭바 관용구를
쓰는데 인상만 갈렸다 — 앱은 떠 보이는 반투명 알약, 웹은 가장자리에 붙은 불투명 흰 막대.

### 구현
`MobileTabBar` 를 **흐름 자식으로 둔 채** 알약으로 만든다 — 좌우 12px·아래 10px 여백,
`--chip-radius`(기존 토큰), 반투명 배경(`--glass-bg`), `--white-color-60` 테두리,
`--shadow-md`. safe-area 는 탭바 자신의 아래 여백이 받는다.

### 처음 시도했다가 되돌린 것 (중요)
탭바를 `position: absolute` 로 **콘텐츠 위에 띄워** 반투명 뒤로 예약 카드가 비치게 했다.
코드리뷰가 치명 결함 2건을 잡아 접근을 되돌렸다.

1. **월 캘린더 마지막 주 행이 영구히 가려졌다.** `MonthWrap` 은 `overflow: hidden` 이라
   스크롤 자체가 없다. 보정으로 넣은 `StyledMain` 의 `padding-bottom` 은 닿지 못했고,
   탭바가 터치까지 먹어 **그 날짜를 누를 수도 없었다.** 앱 기본 진입 화면이다.
2. **그 `padding-bottom` 자체가 작동하지 않았다.** 전역 `box-sizing` 리셋이 없어
   `<main>` 은 content-box 인데 `height: 100%` 가 함께 걸려, 패딩이 요소를 키우고
   초과분은 `StyledContent` 의 `overflow: hidden` 에 잘렸다. 여백이 화면 밖에 생겼다.
   설령 box-sizing 을 고쳐도, `display:flex` + stretch 자식 조합에서는 컨테이너
   `padding-bottom` 이 스크롤 끝에 더해지지 않는다(측정으로 확인).

덤으로 둘 더 있었다 — 탭바가 `currValue.full !== null` 안으로 들어가 **로딩 중 사라졌고**,
`position: relative` 래퍼가 스택 순서를 바꿔 **드래그 고스트가 탭바 밑으로 숨었다.**

**교훈:** 떠 있는 바의 "뒤가 비친다"와 "아무것도 가리지 않는다"는 동시에 성립하지 않는다.
비치게 하려면 콘텐츠를 덮어야 하고, 안 가리려면 자리를 비워야 하는데 그러면 비칠 것이 없다.
iOS 는 스크롤 콘텐츠에 inset 을 줘서 푸는데, 웹에선 페이지마다 스크롤 컨테이너가 달라
한 곳에서 줄 자리가 없다. 비치는 효과를 포기하고 흐름 배치를 택했다.

### 리팩토링에서 함께 정리한 것
- `--glass-border` 는 기존 `--white-color-60` 과 **같은 색**이었다(rgba vs hsla 표기만 다름) → 삭제
- `--radius-pill` 은 기존 `--chip-radius` 와 같은 999px → 삭제, 기존 토큰 사용
- `@supports` 폴백과 `--glass-bg-opaque` 삭제 — 저장소의 다른 `backdrop-filter` 13곳은
  전부 폴백 없이 쓴다. 여기만 규약이 달랐다
- `--mobile-tabbar-space`, `StyledMainArea` 래퍼 삭제 — 흐름 배치라 필요 없다

### 검증 (실측)
- 월 캘린더: 마지막 날짜 셀 하단 666px / 탭바 상단 741px — **가림 없음**
- 알약: `border-radius: 999px`, `margin: 0 12px 10px`, `rgba(255,255,255,.72)`
- 데스크톱은 계속 `display: none`
## 진행 중 — 모바일 주별 보기: 예약 카드에 고객명만 (#231, #229 위에 쌓임)

### 배경
모바일 주 뷰는 7칼럼이라 하루가 약 47px다. 이 폭에 시술명·상태·고객명을 다 넣으니
글자가 쪼개졌다 — 실제로 `남성커트 (결제완료) N 최민준`이 네 줄로 접혔다.
좁은 칸에서는 **누가 오는지**만 읽히면 된다. 담당자는 카드 왼쪽 색 막대가 이미 말해주고,
시술명은 일 뷰나 상세에서 본다.

### 왜 렌더 분기(JS)인가 — 제약이 아니라 선택이다
카드는 높이로 표시 단계를 정한다(`cardDetailForHeight`). `full`은 시술 칩이 별도 요소라
CSS로 숨길 수 있지만, `compact`는 `고객명 · 시술명 (상태)`가 한 텍스트 노드라 못 쪼갠다.
**다만 그건 지금 그렇게 써 놨기 때문이지 나눌 수 없어서가 아니다** — span 셋으로 쪼개면
CSS(컨테이너 쿼리)가 닿는다. 그 편이 3일 뷰·태블릿 주 뷰·좁은 데스크톱 창까지 폭이라는
진짜 치수로 한 번에 잡고, 첫 페인트 깜빡임도 구조적으로 없앤다.
이번엔 범위를 좁게 두려고 JS 분기를 택했다. 같은 문제가 또 나오면 그때는 컨테이너 쿼리로 갈 것.

### 구현
- `TimelineReservationCard` — `nameOnly` 옵션. 켜지면 높이와 무관하게 `name` 단계로 내리고
  상태 접미사를 뺀다. **신규 `N` 배지도 뺀다** — 배지(15px)+간격이 들어가면 47px 칸에서
  정작 이름이 잘린다(실측으로 확인).
- `TimelineCluster` — `hideAssignees` 옵션. 담당자 이름표가 칸을 넘겨 `2건예약`까지
  세로로 쪼갰다.
- `Timeline.tsx` — `isNarrow && type === ViewType.Week` 일 때만 둘 다 켠다.
  (`isNarrow`는 #229가 이미 추가한 구독을 그대로 쓴다.)

### 검증
- 모바일 주 뷰: 카드 `최민준` 한 줄, 시술명·상태·N 배지 없음. 묶음은 담당자 배지 없음
- 모바일 일 뷰: 무변경 — 시술 칩·상태·N 배지 그대로, #229 겹침 분할도 살아 있음
- 데스크톱 1024px 주 뷰: 무변경 — `남성커트 (결제완료) N 최민준`, 묶음에 담당자 배지 유지

### 의존
**#229 위에 쌓았다.** #229가 `Timeline.tsx`에 모바일 폭 판정을 추가하는데 같은 파일에
같은 코드를 따로 넣으면 머지 때 충돌한다. **#229 머지 후 이 PR을 머지할 것.**

### 남은 것
묶음의 `2건예약` 글자는 담당자 배지를 뺀 뒤에도 47px 칸에서 두 줄로 접힌다. 이번 범위 밖.
...
### 2라운드 코드리뷰에서 고친 것 (사용자 요청으로 리뷰 사이클 재실행)
- **`HeaderSearchLayer.tsx`의 게이트·매치 판정이 서로 다른 문자열을 봤다.** `trimmedQuery`로
  게이트를 열고 초성·하이라이트도 그걸 쓰는데, 정작 이름·전화 `.includes()`는 트림 안 한 원본
  `query`를 썼다 — 끝에 공백이 남는 모바일 IME 입력에서 게이트는 통과하는데 결과가 전멸했다.
  세 조건 다 `trimmedQuery`로 통일(대소문자 구분 규칙 자체는 안 건드림 — 그건 화면 간 기존 차이라
  이 브랜치 범위가 아니다). 헤드리스 브라우저로 "김민수 "(끝 공백) 입력 → 매치 확인.
- **`AddressCustomerSummary.tsx`의 매치 메모 태그 `key={tag.text}`가 중복 key를 낼 수 있었다.**
  `addTag`(입력 UI)는 중복 텍스트를 막지만 병합·서버 데이터 경로는 그 보장이 없다 → `${tag.text}-${index}`로.
  헤드리스 브라우저로 같은 텍스트의 태그 2개를 만들어 렌더 확인(React key 경고 0건).
- 테스트 공백 보강: `getChosung('')`·서러게이트 페어(이모지) 통과·`caseInsensitive`+초성 조합.
  이모지 테스트는 실제 인덱스 값을 손으로 검산하지 않고 `node`로 실측해 넣었다(UTF-16 코드유닛
  오프셋이라 이모지 뒤 인덱스가 코드포인트 수가 아니라 코드유닛 수만큼 밀린다 — 처음 손으로 짠 기대값이
  틀려서 재확인 후 고쳤다).
- **스킵(근거 남김)**: `<mark>`에 별도 ARIA 처리를 하지 않았다. `<mark>`가 감싸는 텍스트 자체는
  스크린리더가 정상적으로 읽고(정보 손실 없음), "왜 매치됐는지"는 시각적 부가 정보일 뿐 유일한
  전달 경로가 아니다(고객 정보 자체는 색과 무관하게 전부 노출됨) — GitHub·Google 검색 결과도 같은
  패턴. 모든 매치 행마다 "검색어와 일치"를 스크린리더로 반복 낭독하면 목록 탐색이 오히려 방해된다.

### 3라운드 최종 확인
요구사항 3가지(초성검색·하이라이트·메모 노출)가 고객명단·고객레이어 두 화면에 실제로 코드로
연결돼 있는지 처음부터 다시 훑고, `pnpm test`(176건)·`pnpm build`·`pnpm lint`를 재실행해 확인.
실질적인 남은 문제 없음 — 사이클 종료.

### 범위 밖
`Footer.tsx` 죽은 코드는 위 초성 검색 작업에서 이미 발견·제안했고, 별도 이슈(#220, PR #222)로 삭제 완료.
...
### 코드리뷰에서 잡아 고친 것 (실제 브라우저 구동으로 확인)
- **툴팁이 막대 뒤로 깔렸다.** 열에 `position:relative`를 주면서 DOM 상 뒤인 열이 툴팁을 덮었다(`z-index:1`).
  라인 차트 때도 같은 구조였으나 점이 7px이라 드러나지 않았던 것 — 막대로 커지며 보였다.
- **`오늘`(1일) 범위에서 막대가 차트 폭을 통째로 채웠다.** `max-width: 48px` + `justify-content: space-around`.
  space-around 라야 폭 상한이 걸린 구간에서도 막대 중심이 각 1/n 슬롯 가운데에 남아 **툴팁 가로 위치(xRatio)와 어긋나지 않는다.**
- **120일 넘는 커스텀 범위에서 막대가 1px 미만(0.73px)이라 회색으로 뭉갰다.** 그 구간은 gap 0 → 1.73px.
- **매출이 0뿐인 기간**은 빈 격자에 축만 `1원`(`Math.max(…,1)`)으로 남았다 → 빈 상태 문구로(iOS 앱과 동일 처리).
- `seriesLength` 프롭 제거 — `chartPoints`가 곧 series라 항상 같은 값이었다.

### 툴팁 위치 — 비율 추정에서 실측으로
처음엔 기존 방식(비율식)을 그대로 뒀는데, 재보니 **가운데가 아니었다**: 가운데 막대 12px, 첫 막대 61px,
마지막 막대 -43px. `left` 가 `clamp(84px, 74px + ratio*(100% - 74px) - 58px, …)` 로 **툴팁 폭을 116px로 가정**했는데
실제는 164px 였다(`box-sizing: border-box` 가 이 저장소에선 `button`·`input`·`a` 에만 걸려 있어 div 는 패딩이 폭에 더해진다).
비율식으로는 폭을 알 수 없으므로 **막대와 박스를 실제로 재서 px 로 넘긴다**(`showTooltip`).

- 툴팁 폭을 `REVENUE_TOOLTIP_WIDTH`(140px, `box-sizing: border-box` 명시)로 고정해 가둘 기준을 만든다.
- 세로는 `translateY(calc(-100% - 8px))` — **툴팁 높이를 상수로 가정하지 않는다**(기존 `-58px` 가정이 어긋난 원인과 같은 함정).
  카드 위로 넘치지 않게 `top` 만 62px에서 가둔다.
- 가장자리 막대는 툴팁이 카드 안으로 밀리되 **화살표가 막대를 따라간다**(`$arrowLeft`).
- 실측 결과 툴팁 중심 오차 **1px**(반올림), 화살표는 1·7·30·90일 × 첫·중간·끝 전 조합에서 **1px**.
- 덤: `xRatio`·`heightRatio` 계산이 통째로 없어져 `RevenueSection` 의 `useMemo` 하나가 사라졌다(series 를 그대로 넘긴다).

### 색 — 키컬러(`--brand-color`)
막대는 원래 점(dot) 버튼이 쓰던 `--blue-color`(시안)를 이어받았는데, 이 카드의 강조색은 `#2D7FF9`(로열블루)라
**같은 화면에 파랑이 둘**이었다. 선일 때는 시안이 2.25px 선에만 있고 면적은 로열블루라 드러나지 않던 문제.
사용자 결정으로 **키컬러 `--brand-color`(#6526d9)** 로 통일한다.

- 막대 `var(--brand-color)`, hover 는 같은 색상에서 채도만 낮춘 `hsl(261, 40%, 60%)`(강조는 툴팁이 맡는다), 열 배경 `var(--brand-color-bg)`.
- 툴팁 금액·테두리, 카드 테두리·그라디언트도 함께 키컬러 계열로 — **이 세 스타일은 추이 차트 전용**이라 다른 차트에 번지지 않는다(확인함).
- 결제수단·유입 도넛의 팔레트(`#2D7FF9` 등)는 건드리지 않았다. 계열별 구분색이라 키컬러와 별개다.

### 긴 기간 — 가로 스크롤
기간에는 **상한이 없다**. 빠른 범위는 오늘/7일/30일이지만 날짜 입력(`<input type=date>`)에 `min`/`max` 가 없고,
`GET /api/reservations` 도 매장 전체 예약을 조건 없이 준다. 실제로 5년(1,825일)을 고르면 막대가 **0.34px** 라
한 덩어리로 뭉갰다(라인 차트도 같은 범위에서 검은 띠가 됐고, 7px 점들이 겹쳐 클릭은 더 안 됐다).

- 막대에 `min-width: 6px` 를 주고 스테이지를 `overflow-x: auto` 로 — **CSS만으로 자동 전환**된다.
  약 90일까지는 지금처럼 폭에 맞춰 늘어나고, 그 이상은 스크롤(365일=2,554px, 5년=12,774px).
- **축 라벨을 스크롤 콘텐츠 안으로 옮겼다.** 밖에 두면 화면엔 90일치만 보이는데 라벨은 전체 범위를 말해 거짓이 된다.
- 스크롤바가 그래프를 깎지 않도록 높이는 트랙(190px)이 갖고 스테이지는 그만큼만 늘어난다(236px).
- `>120일 → 간격 0` 티어는 제거. 막대 폭은 이제 `min-width` 가 지키므로 간격을 죽일 이유가 없고, 오히려 1px 간격이 있어야 막대가 구분된다.
- 툴팁은 좌표를 실측하므로 **스크롤된 상태에서도 그대로 맞는다**(끝까지 스크롤 후 마지막 막대 hover → 화살표 오차 1px, 확인함).
- 이건 **집계(주/월 묶기)를 대체하지 않는다.** 5년치를 20화면 스크롤하는 건 여전히 비현실적이라, 긴 범위 자동 묶기는 별도 과제로 남는다.

### 기간 상한 1년 (`REVENUE_MAX_RANGE_DAYS = 365`)
스크롤은 "볼 수는 있게" 하는 안전장치일 뿐이라, 애초에 비현실적인 범위를 막는다. 사용자 결정으로 **1년**.

- **값의 근거**: 이 기간은 차트만 쓰는 게 아니라 KPI·일별 목록·**엑셀 내보내기**가 함께 쓴다. 90일로 잡으면
  분기·연간 정산과 그 기간의 엑셀 추출이 막힌다. 365는 *간격*이라 윤년(366일)이 그대로 들어간다(테스트로 고정).
- **두 곳을 막는다.** 날짜 입력은 네이티브 `min`/`max`(각 입력을 상대 날짜의 ±365 안으로) — 피커가 범위 밖을 회색 처리한다.
  다만 **직접 타이핑한 값은 그대로 들어오므로**(브라우저는 `rangeOverflow` 로 표시만 한다) `settings.tsx` 의 쿼리 파싱에서
  `clampRevenueRange` 로 자른다. 날짜 변경이 전부 `replaceQuery` → URL → 파싱을 거치므로 여기가 유일한 관문이고, 북마크된 링크도 같이 막힌다.
- 자를 때는 **끝(최근)을 남기고 시작을 당긴다** — 최근을 보려던 의도일 가능성이 높다. 뒤집힌 기간은 순서를 건드리지 않고 간격만 자른다.
- `이전/다음 기간` 이동은 현재 간격을 유지하며 움직이므로 상한을 넘길 수 없다.

### 검증
`pnpm lint`(신규 지적 0 — 기존 에러 26건은 손대지 않은 파일) · `tsc --noEmit` 0건 · 테스트 152개 통과 · `next build` 성공.
차트는 순수 모듈(`client/features/**`)이 아니라 테스트 추가 대상이 아니고, 대신 **실제 컴포넌트를 브라우저에 띄워
hover/포커스/클릭·1·7·30·180·365일 범위를 구동해 확인**했다(하네스는 커밋하지 않음).
