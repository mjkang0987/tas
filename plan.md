...
### 구현
휴업일 판정은 `storeSettings.closedDates.includes(dateKey)` 한 줄(둘 다 `YYYY-MM-DD`,
`toDateKey` 형식 — 횡단 규칙 1번). 서버(`booking-helpers.ts:evaluateDateWindow`)와 같은 방식.
- `client/components/calendar/views/Month.tsx` — 월별 뷰 셀에 `휴무` 배지(`LabelBadge` 재사용,
  `$tone="danger"`) + 셀 배경 옅은 적색(`--danger-bg`).
- `client/components/calendar/views/Timeline.tsx` — 일별·주별·3일 뷰는 전부 `Timeline` 하나를
  지나가므로 여기 한 곳에 붙이면 3개 뷰가 덮인다. 열 배경 틴트 + 상단 `휴무` 배지.

### 영향 파일
`client/components/calendar/views/Month.tsx`, `client/components/calendar/views/Timeline.tsx`.
(순수 모듈 `client/features/**` 변경 없음 → 단위 테스트 의무 대상 아님. 표시 전용 변경.)

### 검증 결과 (프로덕션 빌드 + 헤드리스 Chromium, 게스트 모드)
오늘(2026-08-18)과 +2일(08-20)을 휴업일로 심은 로컬 스냅샷으로 각 뷰를 띄워 확인했다.

| 뷰 | 결과 |
|---|---|
| 월별 1440px | 휴업일 2일 + 일요일(정기휴무) 6칸, 셀 틴트 + 배지 한 줄 |
| 주별 1440px | 해당 열 전체 틴트 + 상단 배지, 한 줄 |
| 일별 1440px | 하루 전체 틴트 + 상단 배지 1개 |
| 월별 390px | 최악 배지 30px / 셀 55.7px — 여유 12.3px |
| 주별 390px | 최악 배지 30px / 열 49.4px — 여유 9.7px |
| 3일 390px | 최악 배지 30px / 열 115.3px — 여유 42.7px |

**모바일 폭이 이번 작업에서 제일 까다로웠다.** 처음엔 배지를 날짜 헤더에 인라인으로 넣어
390px 월별에서 셀 밖으로 12px 넘쳤고(→ 헤더 아래 한 줄로 분리), 문구가 `정기휴무`(4글자)로
길어지자 390px **주별에서 4.3px 넘쳤다**(배지 58px vs 열 49.4px). 폰트 토큰 하한이 10px
(`--tiny-font`, 횡단 규칙 9번)이라 줄여서는 해결되지 않는다. `max-width`로 CSS 줄바꿈에
맡겼더니 한국어는 글자 사이 어디서나 끊겨 **'정/기/휴/무' 세로 한 줄**이 됐다.
→ 라벨을 조각 배열(`STORE_CLOSED_LABEL_PARTS`)로 두고 ≤640px에서 `flex-direction: column`으로
쌓아 '정기 / 휴무' 두 줄로 만든다. 데스크톱은 그대로 한 줄.

빌드·`tsc --noEmit`·eslint(변경 파일, 신규 지적 0 — `cardDetailForHeight` 경고는 기존 건)·
`pnpm test`(85건)·`test:required` 게이트 통과.

### 리스크 / 남는 것
- 예약이 있는 날은 배지 한 줄만큼 목록이 아래로 밀린다(390px 월별에서 배지+예약+`전체(1)`가
  한 셀에 같이 들어가는 것을 확인).
- 320px(iPhone SE 1세대급)은 실측하지 않았다. 두 줄 배지가 30px이라 셀 ~45px에도 들어가지만
  확인은 안 했다.
- 실기기(Safari·iOS) 확인은 안 했다. 배경색·배지뿐이라 위험은 낮다.
- **앱(tas-ios)은 판정 로직·단위 테스트까지만** 넣었고 화면 표시는 배지 뷰 승인 대기 중이다.
