...
## 진행 중 — 짧은 예약(5·10분) 카드가 읽히게 (타임라인 배율 배선)

> 판정 모듈(`features/reservations/timeline-scale.ts`)은 #195에 사전작업으로 들어갔다. 이제 화면에 배선한다.
> 결정 근거·시안: 세션 산출물 문서(안 A+B 결합 — 소요시간이 배율을 정하고, 짧을수록 카드 내용을 줄인다).

### 무엇이 문제였나
- 시간당 100px 고정이라 10분 예약 16.7px, 5분 8.3px — 글자 한 줄(약 17px)도 안 들어간다.
- 눈금이 30분까지만 있어 10분 예약의 시작점을 눈으로 못 읽는다.
- 드래그 스냅이 `Math.round(m / 30) * 30`로 **30분 하드코딩** — 옮기면 시각이 틀어진다.

### 구현 방침
- 매장 단위 = 서비스 카탈로그 최소 소요시간(내림). 배율: 5분 200px · 10분 120px · **15분 이상 100px(현행 유지)**.
- 카드 내용은 **매장 단위가 아니라 그 카드의 높이**로 정한다 — 예약별 수동 소요시간이 있어 30분 매장에도 짧은 카드가 생긴다.
- 배율로 못 잡는 카드는 최소 높이 16px가 받는다. 넘치는 글자는 **말줄임**.
- 마우스 있는 기기는 hover 시 자연 높이로 펴서 전체 정보(`:focus-visible`도 동일, 드래그 중엔 억제).

### 영향 파일
- `hooks/useTimelineScale.ts`(신규), `views/Timeline.tsx`, `views/TimelineTitle.tsx`,
  `views/useTimelineDrag.ts`, `views/timelineInteractions.ts`, `views/TimelineReservationCard.tsx`, `ui/Buttons.tsx`

### 리스크 (검증 설계의 근거)
- **배율을 4곳이 나눠 쓴다** — 하나만 옛 상수를 쓰면 축과 카드가 어긋나 예약 시각이 틀리게 잡힌다. 최대 회귀 위험.
- 30분 단위 매장은 **좌표가 지금과 완전히 동일**해야 한다.

### 검증
- 좌표 대조(`getBoundingClientRect` vs 계산값), 축↔카드 정렬, 단위별(30/10/5분) 시드 3회,
  드래그 스냅 단위, 클릭 생성 시각, 현재시각 바, 일·3일·주 3개 뷰, hover/focus, 말줄임.
- 스키마 변경 없음.
