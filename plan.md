...
## 진행 중 — 모바일 하단 탭바를 플로팅 유리 스타일로 (#226)

### 배경
iOS 앱(`tas-ios`)과 웹 모바일을 **같은 시드 데이터**로 나란히 띄워 비교했다(양쪽 모두
`takeaseat.local-db.v1` 키를 쓰는 게스트 모드라 동일 데이터 주입이 가능했다).
하단 탭바의 인상이 크게 갈렸다 — 앱은 화면 아래 떠 있는 반투명 알약, 웹은 가장자리에 붙은
불투명 흰 막대(`MobileTabBar.tsx:68-80`).

웹 모바일도 앱과 **같은 하단 탭바 관용구**를 이미 쓰고 있다. 조작 방식이 아니라 시각언어만
어긋난 것이므로, 스타일만 맞추면 두 화면이 같은 제품으로 읽힌다.

### 범위
디자인 요소만 — 라운딩 · 플로팅 · 투명도. 탭 **항목** 구성(앱 3개 / 웹 4개)은 건드리지 않는다(별도 건).

### 구현 방식
현재 구조는 `Header → Main → FooterAd → MobileTabBar`이고, 탭바는 떠 있는 게 아니라
**플렉스 자식으로 자리를 차지**한다. 그래서 스타일 교체만으로는 플로팅이 되지 않는다.

탭바를 광고 배너 **위**(콘텐츠 영역)로 옮겨 `position: absolute`로 띄운다:
- 반투명 배경 뒤로 **예약 카드가 비친다**(광고 위에 띄우면 흰 배경만 비쳐 유리 느낌이 죽는다)
- 광고는 흐름상 맨 아래에 남아 **가려지지 않는다** — AdSense 조회가능성 문제를 피한다

### 영향 파일
- `client/components/layout/MobileTabBar.tsx` — 알약 모양·반투명·블러·그림자·선택 하이라이트
- `client/components/layout/LayoutComponent.tsx` — 탭바 위치 이동, `StyledContent` 포지셔닝 기준,
  `StyledMain` 하단 여백(없으면 마지막 예약이 탭바에 영구히 가림), safe-area를 맨 아래가 되는 광고로 이전
- `client/styles/globalStyle.ts` — 유리 배경·블러 토큰 추가. 기존 `--sticky-backdrop`은
  `blur(.8px)`로 이 용도엔 부족하다. 리터럴 금지 규약에 따라 토큰으로 넣는다

### 리스크
- **광고 위치가 화면 맨 아래로 내려간다.** 지금은 탭바가 그 자리였다. 정책 위반은 아니지만
  클릭률 변동 가능성이 있다. 되돌리기는 쉽다(JSX 순서 한 줄).
- `backdrop-filter` 미지원 브라우저 → `@supports`로 불투명 배경 폴백을 둬 글자 가독성을 지킨다.

### 예상 결과
모바일(≤640px)에서 탭바가 떠 보이고 스크롤 시 예약 카드가 뒤로 비친다. 광고·마지막 예약 모두
가려지지 않는다. 데스크톱(>640px)은 무변경(탭바 계속 숨김).
