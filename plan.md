...
## 진행 중 — 고객 병합 미리보기에 판단 정보 표시

> 배경: 고객명단(`/address`)에서 2명 이상 선택 후 "병합"을 누르면 뜨는 미리보기가 **이름·연락처·기준/삭제 배지**만 보여준다. 동명이인이거나 이름이 마스킹된(`*` 포함, 네이버 유입) 고객이 섞이면 어느 쪽을 기준으로 삼아야 하는지 판단할 근거가 화면에 없다. 기준 고객은 이름·연락처가 유지되는 쪽이라 잘못 고르면 되돌리기(분리) 전까지 잘못된 번호가 남는다.

### 범위
- `client/components/address/AddressContent.tsx` 미리보기 항목에 **예약 건수 · 적립금**을 연락처 옆에 추가. 데이터는 이미 컴포넌트가 받는 `reservationsByCustomer`·`customer.points`로 충족 — API·스키마 변경 없음.
- 연락처 포맷 인라인 정규식(11자리만 처리)을 공용 `formatTel`로 교체 — 10자리 번호도 정상 표기.
- 항목이 길어지므로 `flex-wrap`으로 줄바꿈 허용(모바일).

### 제외
- 고객명단 행의 "병합됨" 배지·이전 연락처 노출은 이번 범위 밖(merge-history 조회 API 확장 필요).

### 검증
- ✅ `next build` 성공(Compiled successfully).
- ⬜ 실제 화면에서 병합 미리보기 구동 확인(로그인·데이터 필요).
