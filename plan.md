...
| 뷰 | 결과 |
|---|---|
| 월별 1440px | 18·20 셀 배경 틴트 + `휴무` 배지 2개 |
| 주별 1440px | 18·20 열 전체 틴트 + 상단 배지 2개 |
| 일별 1440px | 하루 전체 틴트 + 상단 배지 1개 |
| 월별 390px(모바일) | 배지 2개, 셀(55.7px) 안에 32px — 넘침 없음 |

빌드·`tsc --noEmit`·eslint(변경 파일, 신규 지적 0 — `cardDetailForHeight` 경고는 기존 건)·
`pnpm test`(85건)·`test:required` 게이트 통과.

### 리스크 / 남는 것
- 월별 뷰 좁은 셀에서 배지가 날짜 숫자·추가 버튼과 자리를 다툰다 → **헤더에 끼우지 않고 헤더 아래
  한 줄**로 뺐다(헤더에 끼웠을 때 390px에서 셀 밖으로 12px 넘쳤다). 예약이 있는 날은 배지 한 줄만큼
  목록이 아래로 밀린다.
- 실기기(Safari·iOS) 확인은 안 했다. 배경색·배지뿐이라 위험은 낮다.
- 정기 휴무(요일)는 여전히 캘린더에 안 보인다 — 원하면 같은 판정 자리에 한 줄로 얹는다.
