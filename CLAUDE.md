...
- **`main` 직접 푸시 금지 — 사람도 자동화도.** `main` 푸시가 곧 Cloud Build → Cloud Run **배포**다.
  스케줄 워크플로·봇도 예외가 아니다. 브랜치에 푸시하고 **PR을 만들어** 사람이 머지하게 한다.
  > 실제 사례: `update-default-prices.yml`이 생성 결과를 `main`에 직접 푸시하면, 반년마다 자정에
  > **무인 프로덕션 배포**가 일어난다. 게다가 그 내용은 LLM이 생성한 코드다.
- **PR base는 항상 `main`.** `develop`을 base로 하는 PR을 만들지 않는다.
  `develop`은 `main`으로 흘러들지 않으므로 **머지해도 배포에 도달하지 못한다** — 조용히 실패한다.
  기존에 `develop`을 base로 열린 PR이 있으면 base를 `main`으로 옮기고 `Main 재동기화`를 거친다.
...
- **`main` 직접 푸시 금지 · PR base는 항상 `main`.** 자동화(스케줄 워크플로·봇)도 `main`에 푸시하지 않고 PR을 만든다. `develop` 대상 PR은 만들지 않는다 — 머지해도 배포에 도달하지 못한다. (`Development Workflow` 참조)
