...
- **작업 분할 및 브랜치 생성:** 작업 요청 시 가장 작은 단위의 이슈로 나누고, **`main` 최신본을 기준으로**
  ...
  > **왜 `develop`이 아니라 `main`인가** — feature를 `main`에 개별로 배포하는 구조에서 `develop`을 기준으로 따면,
  > 검증한 조합(`develop` = 내 것 + 남의 미출시분)과 배포되는 조합(`main` + 내 것)이 **달라진다.**
  > 남의 미출시 변경에 기대는 코드를 짜도 `develop`에선 통과하고 `main`에서만 깨진다.
  > `main`에서 따면 feature 브랜치가 컴파일하는 조합이 곧 배포될 조합이라 이 구멍이 닫힌다.
...
- **통합 사전경보 (선택):** 동시 진행 중인 다른 feature가 있으면 `develop`에 머지해 **충돌 여부만** 미리 본다.
  배포 경로가 아니며 **게이트도 아니다.** `develop`은 `main`으로 흘러들지 않으므로 주기적으로 `main`을 받아 동기화한다.
...
6. **재검증** — 리팩토링 후 다시 빌드·검증.
   1. **(선택) 통합 사전경보** — 동시 진행 건이 있으면 `develop`에 머지해 충돌만 확인한다. 게이트가 아니며, 여기 통과를 검증으로 치지 않는다.
   2. **Main 재동기화(필수)** — `origin/main`을 병합하고 **4~6을 다시 통과**시킨다. 브랜치를 딴 뒤 `main`이 움직였으면 검증 기준이 낡은 것이다.
