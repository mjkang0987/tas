...
| `customers/chosung.ts` | - | 한글 초성(첫소리) 검색 — `getChosung`(음절→초성, 비한글은 그대로 통과)·`isChosungQuery`(입력이 초성 자모로만 구성됐는지)·`matchesChosung`(초성 질의를 이름의 초성열에 부분일치, 초성 질의가 아니면 항상 false). 고객명단(`pages/address.tsx`)·헤더 고객검색 레이어(`layout/HeaderSearchLayer.tsx`)가 기존 이름 매칭에 OR로 추가해 재사용. 두 호출부는 대소문자·트림 규칙이 이미 서로 달라 통일하지 않았다 |
