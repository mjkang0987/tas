...
[^33]: 후보가 **1명뿐이면 번호가 있어도 제안한다** — 고를 것이 없으니 갈릴 여지도 없다. 배제는 후보가 2명 이상이고 그중 서로 다른 번호가 2종 이상일 때뿐이고, 그 경우는 주소록에서 사람이 직접 골라 합치는 **수동 병합**에 맡긴다. 마스킹 고객은 네이버 유입이라 번호가 항상 비어 있으므로, 비교 대상은 언제나 실명 후보들끼리다.
