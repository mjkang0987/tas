...
| `/sitemap.xml` | `sitemap.xml.tsx` | 색인 대상 공개 경로 목록(`/`·`/terms`·`/privacy`). **예약 서브도메인에서는 404**[^38] |
| `/robots.txt` | `robots.txt.tsx` | **호스트별로 다른 내용**을 준다 — `public/` 정적 파일이 아니다[^38] |
...
- **고객 예약 호스트 분기**(`auth()` 밖, 점검 모드 다음): 요청 호스트(`x-forwarded-host` 우선 → `host`)로 판정.
...
| `settings/revenue/` | 매출 관리 | `RevenueSection.tsx`(+`.styles.ts`, 순수 차트 로직은 `revenueChartUtils.ts`), `RevenueChartGrid.tsx`(차트는 전부 **CSS(div/버튼)** — 추이=세로 막대, 비중=`conic-gradient` 도넛. SVG 차트는 쓰지 않는다)[^42], `RevenueKpiGrid.tsx`, `RevenueFilters.tsx`, `RevenueMetricModal.tsx`, `RevenueReservationList.tsx`, `RevenueDailyList.tsx`, `RevenueDailyDetailModal.tsx`, `revenue-styles.ts`/`revenue-chart-styles.ts` |
| `address/` | 고객 명단 | `AddressContent.tsx`, `AddressCustomerRow.tsx`, `AddressCustomerSummary.tsx`, `AddressCustomerRecharge.tsx` |
| `ui/` | 공통 UI | `Buttons.tsx`, `Icons.tsx`, `PageHero.tsx`, `SeoHead.tsx`(title·description·canonical·`noindex`), `ServiceChip.tsx`, `AssigneeLabel.tsx`/`ColorTag.tsx`(담당자 색상 배지), `LabelBadge.tsx`(tone×shape 배지), `ReservationStatusBadge.ts`(예약 상태 배지), `ReservationInfoCard.tsx`, `CsFooter.tsx`(고객센터 푸터 공통), `GuestNotice.tsx`, `FieldError.tsx`, `FormControls.ts`, `HighlightMatch.tsx`(검색 매치 구간을 `<mark>`로 감싼다 — `features/customers/search-highlight.ts` 소비), `LoadingOverlay.tsx`+`Spinner.tsx`+`RunningDino.tsx`(로딩 표시 — **전면 오버레이**는 전부 `LoadingOverlay` 하나를 지나간다)[^29] |
...
[^3]: 마스킹 이름 고객 병합 제안 모달 (게스트 모드에서는 비활성). 선택 컨트롤은 라디오 하나뿐이고 후보가 2명 이상일 때만 뜬다. 클릭 영역은 라디오와 그 이름까지 — 카드 전체를 누르면 기준이 바뀌던 동작이 오조작을 불렀다. 기준 고객은 **배경색만으로** 가른다(파란 틴트 10% + 테두리 45%) — 카드 안에 예약 카드가 또 들어가 테두리가 겹치는 구조라 빗금·액센트 바 같은 장식을 더하면 그 경계들과 싸운다. 예전엔 강조가 `hasChoice && isTarget` 조건이라 **후보가 1명일 때 아예 안 걸려** 사라질 카드와 남을 카드가 똑같은 회색이었다
...
[^37]: **루트(`/`)를 색인 가능하게 만든 이유** — Google Search Console 이 "발견됨 – 현재 색인이 생성되지 않음"을 띄웠다. 그전 구조에서 익명 방문자는 루트에서 **빈 앱 셸을 200 으로** 받고, `_app.tsx` 가 하이드레이션 후에야 JS 로 `/about` 으로 옮겼다. 크롤러 입장에서 루트는 내용이 없는 URL 인데, 외부 링크가 붙는 곳은 대개 루트다 — 사이트에서 가장 링크가 많은 URL 이 크롤 가치 0 이었다. **서버 리다이렉트(`/` → `/about`)는 일부러 택하지 않았다.** 그러면 루트가 '색인할 것이 없는 URL' 로 확정되고 사이트맵에 `/` 를 넣는 것도 해가 된다(GSC 가 "페이지에 리디렉션이 있음"으로 제외). 대신 루트가 소개 내용을 직접 200 으로 응답한다. ⚠️ **서버는 게스트를 동의 쿠키(`tas-guest-terms`)로만 알아본다** — localStorage 를 못 보기 때문이다. 쿠키만 지워지고 게스트 데이터가 남으면 소개 화면이 잘못 뜨는데, 그 상태로 두면 `_app` 의 "이전 데이터 불러오기"를 눌러도 **캘린더로 갈 수 없다**(앱 셸이 서버에서 오지 않았으므로). 그래서 `Landing` 이 쿠키를 되살리고 한 번만 새로고침한다 (세션 플래그로 무한 새로고침 방지). 또 하나 — `LayoutComponent` 의 캘린더 URL 동기화 효과는 랜딩에서 **반드시 꺼야 한다**. 안 끄면 루트가 `/month/YYYY/M` 으로 정규화돼 색인 대상 URL 이 주소창에서 사라진다(실제로 그렇게 났다). 익명이 `/month/2026/8` 같은 캘린더 URL 로 직접 들어와도 소개 화면이 뜬다 — `next.config.mjs` 가 그 경로를 `/` 로 rewrite 하기 때문이다(`robots.txt` Disallow + canonical `/` 라 색인 영향은 없다). **공유 캐시(`s-maxage`)는 열지 않는다** — Cloudflare Worker 를 지나므로 HTML 캐시 규칙이 있으면 익명 소개 화면이 로그인 사용자에게 돌아갈 수 있고, CF 는 `Vary: Cookie` 로 구분하지 않는다. **`/about` 은 없앴다** — 같은 내용을 두 URL 에 두면 크롤 예산이 갈리고(이 사이트의 문제가 바로 크롤 우선순위다), exempt 목록 4곳에 특례가 남는다. 다만 **그냥 지우면 무한 리다이렉트가 난다** — `_app` 가드가 "동의 쿠키는 있는데 로컬 데이터가 없는" 방문자를 루트에서 `/about` 으로 보내고 있었기 때문이다(`/about` → `/` → 앱 셸 → 가드 → `/about` …). 그래서 **가드 목적지를 `/login` 으로 먼저 바꾼 뒤** 페이지를 지웠다. 그 방문자에게 소개를 보여줄 이유도 사라졌다 — 소개는 루트가 맡고, 그는 방금 루트에서 온 사람이다. **색인 자체는 이 변경만으로 보장되지 않는다** — "발견됨"은 크롤링 우선순위 문제이고, 사이트맵 제출·색인 생성 요청·외부 링크가 따로 필요하다.

[^38]: **`robots.txt`·`sitemap.xml` 을 호스트별로 나눈 이유** — `public/robots.txt` 로 두면 **예약 서브도메인(`book.takeaseat.co.kr`)도 같은 파일을 받는다.** `proxy.ts` 의 `handleBookingHost` 가 확장자 있는 경로를 그대로 통과시키기 때문이다(실제로 그랬다). 그러면 세 가지가 어긋난다. ① 예약 호스트는 **루트 바로 아래가 매장 슬러그**라 `Disallow: /month`·`/login`·`/settings` 같은 메인 사이트 규칙이 **슬러그를 막는다**(슬러그가 `month` 인 매장은 통째로 크롤 차단). ② `Sitemap:` 이 다른 호스트를 가리키고, 예약 호스트의 `/sitemap.xml` 도 메인 URL 목록을 200 으로 줘 검색엔진이 거부하는 사이트맵이 된다(그래서 404 로 바꿨다). ③ `Allow: /` 라 **예약 확인 링크(`/{slug}/r/{token}`)까지 색인 대상**이었다 — 토큰이 어딘가 공개되면 고객 예약 내용이 색인될 수 있다. ⚠️ **토큰 페이지는 `robots.txt` 로 막지 않고 페이지의 `noindex` 로 뺀다.** 크롤을 막으면 크롤러가 그 태그를 읽지 못해 오히려 URL 만 남은 색인이 생긴다. 그리고 **`noindex` 는 로딩 분기에도 있어야 한다** — 예약을 클라이언트에서 받아오므로 **로딩 상태가 곧 서버 HTML** 이고, 거기 빠지면 JS 를 실행하지 않는 크롤러는 태그를 못 본다. 매장 예약 페이지(`/{slug}`) 자체는 색인을 **열어 둔다** — 매장 노출에 도움이 되고, 오너가 공개 예약을 켠 URL 이다. 닫으려면 `robots.txt.tsx` 의 `BOOKING_ROBOTS` 한 줄이면 된다.
...
[^29]: **로딩 표시** — 진행 표시는 `Spinner`(회전 원형) + 문구가 맡고, `RunningDino`는 대기 시간 장식이다(`clipnote`·`clipnote-ios`에서 이식, 4프레임 47×45 스프라이트 `public/dino-run.png`). 붙이는 곳은 **`LoadingOverlay` 한 곳뿐** — **화면을 덮는 로딩은 전부** 이 컴포넌트를 지나가므로(6곳: 부팅 `_app.tsx` `isBooting`, 라우트 전환 `_app.tsx` `RouteLoading`, `/login`, `/consent` 2곳, `StoreSwitcher`) 화면마다 따로 넣지 않는다. **부분 로딩은 대상이 아니다** — 목록 행·패널 안에서 도는 표시(`GuestMigrationLayer`의 `Spinner` 직접 사용, `BookingRequestNotification`·`SNSLinkingSection`·`inquiry`의 인라인 "불러오는 중" 행)는 오버레이를 경유하지 않으며, 창 테두리를 도는 공룡을 거기에 넣는 건 맞지 않는다. 공룡은 창을 상자로 삼아 네 면을 걷고 코너를 도약으로 돈다. **JS가 붙기 전에도 보여야 한다** — 가장 긴 대기인 부팅 오버레이가 정작 하이드레이션 중이라 effect가 아직 안 돈다. 그래서 바닥 왕복을 styled-components `keyframes`로 깔아 두고, effect가 뜨면 `animation: none`으로 끄고 네 면 걷기를 이어받는다(**끄지 않으면 실행 중인 CSS 애니메이션이 인라인 `transform`을 계속 덮는다**). `prefers-reduced-motion`은 CSS·JS 양쪽에서 존중, `aria-hidden` + `pointer-events: none`. 상자는 `absolute inset: 0`이라 오버레이 안쪽을 그대로 쓴다 — 뷰포트 단위를 쓰면 `backdrop="blur"`처럼 패딩이 있는 변형에서 바닥이 상자 밖으로 나간다.
[^42]: **매출 기간 상한 1년**(`REVENUE_MAX_RANGE_DAYS = 365`, *간격* 기준이라 윤년 366일이 들어간다). 90일이 아닌 이유 — 이 기간은 추이 차트만 쓰는 게 아니라 **KPI·일별 목록·엑셀 내보내기가 같은 값을 공유**해서, 짧게 잡으면 분기·연간 정산과 그 기간의 엑셀 추출이 막힌다. 막는 자리는 둘이다: 날짜 입력의 네이티브 `min`/`max`(피커가 범위 밖을 회색 처리)와 `pages/settings.tsx` 의 쿼리 파싱(`clampRevenueRange`). **입력 속성만으로는 부족하다** — 직접 타이핑한 값은 브라우저가 `rangeOverflow` 로 표시만 하고 그대로 통과시키며, 북마크된 URL 도 우회한다. 날짜 변경이 전부 `replaceQuery` → URL → 파싱을 거치므로 그 파싱 지점이 유일한 관문이다. 자를 때는 끝(최근)을 남기고 시작을 당긴다. 상한 안에서도 90일이 넘으면 막대가 6px 밑으로 눌리므로 차트가 **가로 스크롤**로 넘어간다(축 라벨도 스크롤 콘텐츠 안에 둔다 — 밖에 두면 보이는 막대와 라벨이 어긋난다). 긴 범위를 주/월로 **묶는 집계는 미구현**.
...
| `customers/chosung.ts` | - | 한글 초성(첫소리) 검색 — `getChosung`(음절→초성, 비한글은 그대로 통과)·`isChosungQuery`(입력이 초성 자모로만 구성됐는지)·`matchesChosung`(초성 질의를 이름의 초성열에 부분일치, 초성 질의가 아니면 항상 false). 고객명단(`pages/address.tsx`)·헤더 고객검색 레이어(`layout/HeaderSearchLayer.tsx`)가 기존 이름 매칭에 OR로 추가해 재사용. 두 호출부는 대소문자·트림 규칙이 이미 서로 달라 통일하지 않았다 — `layout/Footer.tsx`의 동일 구조 검색 레이어는 어디서도 import되지 않는 죽은 코드라 대상에서 제외 |
| `customers/search-highlight.ts` | - | 검색 매치 하이라이트 구간 계산 — `findMatchRange(text, query, {caseInsensitive})`. 일반 부분일치 우선, 없으면 `chosung.ts` 초성 부분일치. `ui/HighlightMatch.tsx`(신규 — 매치 구간을 `<mark>`로 감싼다, 고객명단·헤더 검색 레이어 공용)가 소비 |
...
| `useCustomerMergeSuggestion.ts` | 마스킹 이름 병합 제안 감지·큐 (게스트 모드 제외). 규칙은 `features/customers/merge-suggestion.ts`. 재감지는 고객 목록의 **내용**(id·이름·연락처 시그니처)이 바뀔 때 — 수만 보면 이름 수정에 반응하지 못한다 |
...
**한 건은 언제나 마스킹 1명 : 실명 1명.** 후보가 여럿이면 묶지 않고 후보 수만큼 별건으로 내고, 큐가 하나씩 "이 사람이 맞나요?"를 묻는다.[^33]

| 고객 | 제안 | 이유 |
|---|---|---|
| `김민수` + `김*수` | 1건 | |
| `이상민` + `이승민` + `이유민` + `이*민` | **3건** | 후보 3명 → 각각 1:1 로 순차 |
| `김민수`(A) + `김민수`(B) + `김*수` | **2건** | 동명이인도 구분하지 않는다 — 똑같이 1:1 |
| `김민수` + `김민*` + `김*수` | **2건** | 마스킹이 2명 → 각각 별도 제안 |
