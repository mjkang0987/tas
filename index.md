...
| 폴더 | 역할 | 주요 파일 |
|------|------|----------|
| `calendar/views/` | 캘린더 뷰 (일/주/월/년/타임라인) | `Calendar.tsx`, `Day.tsx`, `Week.tsx`, `Month.tsx`, `Timeline.tsx`, `TimelineCluster.tsx`(중복예약 클러스터 — 담당자 배지 표시). 휴업일 표시는 `Month.tsx`·`Timeline.tsx` 두 곳[^30] |
...
[^30]: **휴무(임시 휴업일·정기 휴무) 캘린더 표시** — 설정 > 매장관리의 `closedDates`(특정 날짜)·`closedWeekdays`(매주 요일)를 캘린더에 **옅은 배경 틴트 + 테두리**로 표시한다(임시=`--danger-bg`/`--danger-border`, 정기=`--neutral-bg`/`--neutral-border`). 테두리는 `border`가 아니라 **inset box-shadow** — border 를 주면 셀·열 크기가 1px 밀려 달력 격자와 시간축이 어긋난다. 판정은 순수 함수 `getStoreClosedKind`(`features/store-settings/model.ts`) 한 곳이고 둘 다 해당하면 임시 휴업일이 이긴다. 요일 인덱스는 앱 공통 `0=월 … 6=일`인데 JS `getDay()`는 `0=일`이라 `(getDay()+6)%7`로 바꿔야 한다 — 틀리면 **하루씩 밀린 요일에 휴무가 붙는다**(`closed-kind.test.ts`가 이 회귀를 잡는다). **붙이는 곳은 두 곳뿐** — 월별은 `Month.tsx`, 일별·주별·3일은 전부 `Timeline.tsx`를 지나간다(년별은 월 단위 셀이라 대상 아님). **글자(배지)를 쓰지 않는 이유는 모바일 폭이다** — 배지안은 390px 주별에서 열 49.4px에 58px로 넘쳤고, 폰트 토큰 하한이 10px(횡단 규칙 9번)이라 줄여서 해결되지 않으며, CSS 줄바꿈에 맡기면 한국어는 글자 사이 어디서나 끊겨 '정/기/휴/무'로 쪼개진다. 틴트·테두리는 폭을 먹지 않아 이 문제가 구조적으로 사라진다. **대신 색만으로는 스크린리더에 아무것도 전달되지 않으므로** 문구(`STORE_CLOSED_LABEL`)를 전역 `.a11y`(시각적 숨김) 클래스로 DOM에 남긴다. `title`(툴팁)은 **월별 셀에만** — 타임라인 래퍼는 하루 열 전체라 자식이 title을 물려받아 예약 카드 위에서도 툴팁이 뜬다. **표시만 하고 막지는 않는다** — 휴무일에도 예약 생성·이동·드래그는 그대로이며, 실제로 막히는 것은 고객 공개 예약 페이지뿐이다(`evaluateDateWindow` reason `closed`).
