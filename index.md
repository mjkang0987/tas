...
| `김민수`(A) + `김민수`(B) + `김*수` <br>(**아무도 번호 없음**) | 1건 (후보 2명) | 실명 이름이 1종. 누구인지는 라디오로 고른다 |
| 위 조합 중 **한 명이라도 번호가 있으면** | 0건 | 자동 제안 대상이 아니다 → **수동 병합**[^33] |
...
[^33]: 판정 대상은 **마스킹 고객까지 포함한 그룹 전체**이고, `normalizeTel` 로 숫자만 남겨 본다(공백·하이픈만 든 값은 번호가 아니다). 번호가 붙는 순간 자동으로 판정할 수 있는 게 실은 없어서 그렇다 — 같은 번호라도 가족 공유번호일 수 있고(그래서 `CustomerDetail` 도 번호 중복을 차단하지 않는다), 다른 번호라도 번호를 바꾼 같은 사람일 수 있다. 그래서 자동 제안은 **번호라는 단서가 아예 없어 이름만으로 판단할 수밖에 없는 경우**로 좁히고, 번호가 하나라도 붙은 조합은 주소록에서 사람이 직접 보고 고르는 **수동 병합**으로 보낸다. 마스킹 고객은 유입 시점엔 `tel: ''` 이지만 주소록 편집에 마스킹 이름 가드가 없어 나중에 채워질 수 있으므로 함께 본다. 부작용 — 번호를 꼬박꼬박 받는 매장에서는 자동 제안이 거의 뜨지 않는다. 의도된 트레이드오프다.
