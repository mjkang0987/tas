...
[^30]: **휴업일(임시 휴업일) 캘린더 표시** — 설정 > 매장관리에서 지정한 `storeSettings.closedDates`를 캘린더에서 옅은 적색 배경 + `휴무` 배지로 표시한다. 판정은 `closedDates.includes(dateKey)` 한 줄(양쪽 다 `toDateKey` 형식 — 횡단 규칙 1번, 서버 `booking-helpers.ts:evaluateDateWindow`와 동일 방식). **붙이는 곳은 두 곳뿐** — 월별은 `Month.tsx`, 일별·주별·3일은 전부 `Timeline.tsx` 하나를 지나가므로 뷰마다 따로 넣지 않는다(년별은 월 단위 셀이라 대상 아님). **표시만 하고 막지는 않는다** — 휴업일에도 예약 생성·이동·드래그는 그대로 되며(오너가 휴업일에 예외 예약을 잡는 경우가 있다), 실제로 막히는 것은 고객 공개 예약 페이지뿐이다(`evaluateDateWindow` reason `closed`). 정기 휴무(요일, `closedWeekdays`)는 아직 캘린더에 표시하지 않는다 — 같은 판정 자리에 얹으면 된다. 월별 셀은 폭이 ~56px(모바일)이라 배지를 날짜 헤더에 끼우면 셀 밖으로 나간다 → 헤더 아래 한 줄로 둔다.
