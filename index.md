...
| `ui/` | 공통 UI | `Buttons.tsx`, `Icons.tsx`, `PageHero.tsx`, `SeoHead.tsx`, `ServiceChip.tsx`, `AssigneeLabel.tsx`/`ColorTag.tsx`(담당자 색상 배지), `LabelBadge.tsx`(tone×shape 배지), `ReservationStatusBadge.ts`(예약 상태 배지), `ReservationInfoCard.tsx`, `CsFooter.tsx`(고객센터 푸터 공통), `GuestNotice.tsx`, `FieldError.tsx`, `FormControls.ts`, `LoadingOverlay.tsx`+`Spinner.tsx`+`RunningDino.tsx`(로딩 표시 — 모든 로딩 화면이 `LoadingOverlay` 하나를 지나간다)[^29] |
...
[^29]: **로딩 표시** — 진행 표시는 `Spinner`(회전 원형) + 문구가 맡고, `RunningDino`는 대기 시간 장식이다(`clipnote`·`clipnote-ios`에서 이식, 4프레임 47×45 스프라이트 `public/dino-run.png`). 붙이는 곳은 **`LoadingOverlay` 한 곳뿐** — TAS의 로딩 표시가 전부 이 컴포넌트를 지나가므로(부팅 `_app.tsx` `isBooting`, 라우트 전환 `_app.tsx` `RouteLoading`, `/login`, `/consent`, `StoreSwitcher`) 화면마다 따로 넣지 않는다. 공룡은 창을 상자로 삼아 네 면을 걷고 코너를 도약으로 돈다. **JS가 붙기 전에도 보여야 한다** — 가장 긴 대기인 부팅 오버레이가 정작 하이드레이션 중이라 effect가 아직 안 돈다. 그래서 바닥 왕복을 styled-components `keyframes`로 깔아 두고, effect가 뜨면 `animation: none`으로 끄고 네 면 걷기를 이어받는다(**끄지 않으면 실행 중인 CSS 애니메이션이 인라인 `transform`을 계속 덮는다**). `prefers-reduced-motion`은 CSS·JS 양쪽에서 존중, `aria-hidden` + `pointer-events: none`. 상자는 `absolute inset: 0`이라 오버레이 안쪽을 그대로 쓴다 — 뷰포트 단위를 쓰면 `backdrop="blur"`처럼 패딩이 있는 변형에서 바닥이 상자 밖으로 나간다.
