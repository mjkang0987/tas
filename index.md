...
[^30]: **휴업일(임시 휴업일) 캘린더 표시** — 설정 > 매장관리에서 지정한 `storeSettings.closedDates`를 캘린더에서 옅은 적색 배경 + `휴무` 배지로 표시한다. 판정은 `closedDates.includes(dateKey)` 한 줄(양쪽 다 `toDateKey` 형식 — 횡단 규칙 1번, 서버 `booking-helpers.ts:evaluateDateWindow`와 동일 방식). **붙이는 곳은 두 곳뿐** — 월별은 `Month.tsx`, 일별·주별·3일은 전부 `Timeline.tsx` 하나를 지나가므로 뷰마다 따로 넣지 않는다(년별은 월 단위 셀이라 대상 아님). **표시만 하고 막지는 않는다** — 휴업일에도 예약 생성·이동·드래그는 그대로 되며(오너가 휴업일에 예외 예약을 잡는 경우가 있다), 실제로 막히는 것은 고객 공개 예약 페이지뿐이다(`evaluateDateWindow` reason `closed`). **정기 휴무(요일, `closedWeekdays`)도 같은 자리에서 함께 판정**한다 — 문구는 `정기휴무`, 색은 회색(임시 휴업일은 적색). 둘 다 해당하면 임시 휴업일이 이긴다. 요일 인덱스는 앱 공통 `0=월 … 6=일`이고 JS `getDay()`는 `0=일`이라 `(getDay()+6)%7`로 바꿔야 한다 — 여기가 틀리면 **하루씩 밀린 요일에 휴무가 붙는다**(단위 테스트 `closed-kind.test.ts`가 이 회귀를 잡는다). **모바일 폭이 이 표시의 실제 제약이다** — 월별 셀 ~56px·주별 열 ~49px(390px 화면)이라 배지를 날짜 헤더에 인라인으로 넣으면 셀 밖으로 나가고(→ 헤더 아래 한 줄로 분리), `정기휴무` 4글자는 최소 폰트 토큰(10px)으로도 주별 열에 한 줄로 안 들어간다. CSS 줄바꿈에 맡기면 한국어는 글자 사이 어디서나 끊겨 **한 글자씩 세로로 쪼개진다** → 라벨을 조각 배열(`STORE_CLOSED_LABEL_PARTS`)로 두고 ≤640px에서 세로로 쌓아 '정기 / 휴무' 두 줄로 만든다.
