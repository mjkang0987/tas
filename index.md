...
| `김민수`(A) + `김민수`(B) + `김*수` <br>(둘 다 번호 없음 / 번호가 같음) | 1건 (후보 2명) | 실명 이름이 1종이고 번호로도 갈리지 않는다. 누구인지는 라디오로 고른다 |
| `김민수`(A, 번호 O) + `김민수`(B, 번호 X) + `김*수` | 1건 (후보 2명) | 빈 번호는 "다른 번호"가 아니다 — 비교할 값이 하나뿐 |
| `김민수`(01011112222) + `김민수`(01033334444) + `김*수` | 0건 | 번호가 2종 — **동명이인인지 번호를 바꾼 같은 사람인지 자동으로는 못 가른다**[^33] |
...
[^33]: 비교는 **마스킹 고객까지 포함한 그룹 전체**에서 서로 다른 번호가 2종 이상인지로 판정하고, `normalizeTel` 로 숫자만 남긴 뒤 센다(`010-1111-2222` 와 `01011112222` 는 1종). 마스킹 고객은 네이버 유입 시점엔 `tel: ''` 이지만 주소록 편집에 마스킹 이름 가드가 없어 나중에 번호가 채워질 수 있다. 번호가 **1종 이하면** 갈릴 여지가 없으므로 그대로 제안한다 — 후보가 1명뿐이면 그 후보가 번호를 가져도 마찬가지다. 배제된 조합은 주소록에서 사람이 직접 골라 합치는 **수동 병합**에 맡긴다.
