...
- **고객 예약 호스트 분기**(`auth()` 밖, 점검 모드 다음): 요청 호스트(`x-forwarded-host` 우선 → `host`)로 판정.
  ...
  - 메인 도메인(`takeaseat.co.kr`)의 구 경로 `/book/*` → `https://book.takeaseat.co.kr/*` **308**(기배포 링크 보존). `localhost`·`dev.*`·`*.run.app`은 무변경이라 로컬에선 `/book/[slug]`가 그대로 동작
